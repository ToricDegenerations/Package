...
groebnerToricDegenerations=method(Options => {
	minConvention=>true});
...
groebnerToricDegenerations(List,Ideal):=opt->(L,I)->(
...
groebnerToricDegenerations(Ideal):=opt->(I)->(
...
groebnerToricDegenerations(TropicalCycle,Ideal):=opt->(T,I)->(
   groebnerToricDegenerations(computeWeightVectors(T),I)
    )
...
--function to re-embedd the variety adding the missing binomials of cone number n.
computeNewEmbedding=method(Options => {
	minConvention=>true,bruteForce=>true});


computeNewEmbedding(Ideal,TropicalCycle, ZZ):=opt->(I,T,n)->(
    	  WeightN:={};
	  InitialIdeal:={};
	  if opt.minConvention==false then (
...
computeNewEmbedding(Ideal,Ideal):=opt->(I,J)->(

	       MissingBinomials:=findMissingBinomials(J);
	       print MissingBinomials;
	       s:=#MissingBinomials;
	       Y:=symbol Y;
	       H:=symbol H;
 ...
 --function to compute the toric degeneration of a variety  in case there are some  non-prime cones  in the tropicalization of I, it re-emebedds the variety and checks the cones there
 
 findNewToricDegenerations=method(Options => {
	minConvention=>true,bruteForce=>true});

findNewToricDegenerations(HashTable,Ideal):=opt->(H,I)->(
    	       Output:={};
	       S:={};
    	      if (#(getNonPrimeCones H)==0) then (print "No re-embedding can be done since all cones are prime")
	      else(
	      nonPrimeConesT:=getNonPrimeIdeals H;
	      N:={};
	     
	      for i from 0 to #nonPrimeConesT-1 do (
	      N=computeNewEmbedding(I,nonPrimeConesT_i);
	      print N_0;
	      Output=append( Output,"Re-embedding for cone");
	      Output=append( Output,i);
	      Output=append(Output,groebnerToricDegenerations(N_1,N_0))
	      );
	      return Output
      	      )
...
findNewToricDegenerations(List,Ideal):=opt->(L,I)->(
      	   ToricOutput:=(groebnerToricDegenerations(L,I,minConvention=>opt.minConvention))_1;
	   findNewToricDegenerations(ToricOutput,I)
	  
      	        
    )


findNewToricDegenerations(Ideal):=opt->(I)->(
...
findNewToricDegenerations(TropicalCycle,Ideal):=opt->(T,I)->(
      findNewToricDegenerations(computeWeightVectors(T),I,minConvention=>opt.minConvention)
    )
...
doc ///
    ...
    Usage
        groebnerToricDegenerations(L,I)
        groebnerToricDegenerations(T,I)
        groebnerToricDegenerations(I)
    Inputs
...
doc ///
    Key 
        findNewToricDegenerations
    Headline
    	a function that computes toric degenerations coming from a re-embedding V(I') of the variety
    Usage
        findNewToricDegenerations(I)
    Inputs
    ...
    Description
	  ...
	  (see section 4 of the paper). It applies groebnerToricDegenerations to see if there are 
